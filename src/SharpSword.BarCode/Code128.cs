...
namespace SharpSword.BarCode
{
    ...
    public class Code128 {
        ...
        /// <returns>图形</returns>
        public void GetCodeImage(string pText, Encode pCode, Stream target) {
            string _viewText = pText;
            string _text = "";
            ...
            switch (pCode) {
                case Encode.Code128C:
                    _examine = 105;
                    if (!((pText.Length & 1) == 0)) throw new Exception("128C长度必须是偶数");
                    while (pText.Length != 0) {
                        int _Temp = 0;
                        try {
                            int _CodeNumb128 = Int32.Parse(pText.Substring(0, 2));
                        }
                        catch {
                            throw new Exception("128C必须是数字！");
                        }
                        _text += GetValue(pCode, pText.Substring(0, 2), ref _Temp);
                        _TextNumb.Add(_Temp);
                ...
                case Encode.EAN128:
                    ...
                    while (pText.Length != 0) {
                        ...
                        _text += GetValue(Encode.Code128C, pText.Substring(0, 2), ref _Temp);
                        _TextNumb.Add(_Temp);
                        pText = pText.Remove(0, 2);
                    }
                    break;
                default:
                    ...
                    while (pText.Length != 0) {
                        int _Temp = 0;
                        string _ValueCode = GetValue(pCode, pText.Substring(0, 1), ref _Temp);
                        if (_ValueCode.Length == 0) throw new Exception("无效的字符集!" + pText.Substring(0, 1).ToString());
                        _text += _ValueCode;
                        _TextNumb.Add(_Temp);
            ...
            //return _CodeImage;
            _CodeImage.Save(target, ImageFormat.Jpeg);
        }
