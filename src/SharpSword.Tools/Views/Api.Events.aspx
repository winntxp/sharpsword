...
<%  
    var responseDto = this.ActionResult.Data as IEnumerable<EventsAction.ResponseDto>;
    int index = 0;
%>
...
<head>
    ...
    <title>系统事件定义，基于框架SharpSword：<%:ApiVersion.Version %></title>
    <style type="text/css">
        html {
            height: 100%;
            margin: 0px;
...
<body style="font-family: Consolas; padding: 0px; margin: 0px">
    <table style="height: 30px; line-height: 30px; width: 100%; margin: 0px; background: #305c9a; color: #ffffff; font-size: 14px; font-family: 微软雅黑; padding: 0px; border-collapse: collapse;">
        <tr>
            <td style="padding: 0px; font-weight: bold; line-height: 30px; height: 30px; font-size: 14px;">系统事件定义</td>
    ...
    <table class="detail" style="border-collapse: collapse; border: 1px solid #ccc; padding: 0 10px 0 10px; color: #ffffff;">
        <tr style="background: #ff6a00; font-weight: bold; color: #fff; height: 30px;">
            ...
            <th class="l" style="width: 300px">事件名称</th>
            <th class="l" style="width: 300px">程序集</th>
            <th class="l">类型</th>
            <th class="l">订阅类</th>
        </tr>
        ...
        <tr style="background: #ffffff; color: black;">
            ...
            <td class="l"><%:@event.EventType.Name %></td>
            <td class="l"><%:@event.EventType.Assembly.GetName().Name %></td>
            <td class="l"><%:@event.EventType.ToString() %></td>
            <td class="l">
                <% foreach (var eventHandlerType in @event.HandlerTypes)
                    { %>
                <li><%:eventHandlerType.FullName %></li>
                <% } %>
            </td>
        </tr>
        <%} %>
